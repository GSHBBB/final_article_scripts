...
data_file = os.path.join(OUTPUT_DIR, "output_Master_Panel.csv")
...
required_cols = ['REF_AREA', 'TIME_PERIOD', 'True_Resilience', 'Exposure', 'GDP_PC', 'FDI', 'Out_Degree_Centrality', 'Internet_Use']
...
def _coef_cell(res, var):
    if var not in res.params.index:
        return '—', '—'
    coef = res.params[var]
    se = res.std_errors[var] if var in res.std_errors.index else np.nan
    p = res.pvalues[var] if var in res.pvalues.index else np.nan
    coef_txt = f"{coef:.4f}{_stars(p)}"
    se_txt = f"({se:.4f})" if not pd.isna(se) else '—'
    return coef_txt, se_txt
...
def _single_model_table_lines(model_title, res, controls_flag, countries_count, row_spec):
    ...
    lines.append('')
    lines.append('注：括号内为国家层面聚类稳健标准误（Clustered SE at Country Level）。所有模型均包含个体固定效应与年份固定效应，常数项已被固定效应吸收，不单独报告。')
...
def export_combined_markdown_table(path_md):
    ...
    lines.append('被解释变量：ln_Resilience_Inv（网络韧性反转对数指数）')
    lines.append('')
    lines.append('| 变量 | (1) 全样本 | (2) 全样本 | (3) 边缘组 Bottom 33% | (4) 核心组 Top 33% |')
    lines.append('|---|---:|---:|---:|---:|')
    ...
    lines.append('| Year FE | Yes | Yes | Yes | Yes |')
    lines.append('| Controls | No | Yes | Yes | Yes |')
    lines.append('| SE Cluster | Country | Country | Country | Country |')
