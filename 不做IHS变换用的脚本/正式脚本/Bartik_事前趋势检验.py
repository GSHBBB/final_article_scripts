...
# 保存为高分辨率图片（4096x2160）
diag_fig_path = os.path.join(OUTPUT_DIR, 'output_Bartik_事前趋势检验_诊断图.png')
plt.savefig(diag_fig_path, dpi=216, bbox_inches='tight', facecolor='white')
print(f"\n✓ 诊断图已保存：{diag_fig_path}")
print(f"  • 输出分辨率：4096 × 2160 像素")
print(f"  • 中文字体：已启用 (SimHei)")
plt.show()

# 关键结果导出
summary_path = os.path.join(OUTPUT_DIR, 'output_Bartik_外生性检验结果.csv')
summary_df = pd.DataFrame([
    {
        'sample_n': n_obs,
        'coef_exposure_2015': exposure_coef,
        'se_exposure_2015': exposure_se,
        't_exposure_2015': exposure_tstat,
        'p_exposure_2015': exposure_pval,
        'ci95_lower': exposure_ci_lower,
        'ci95_upper': exposure_ci_upper,
        'r_squared': r_squared,
        'adj_r_squared': adj_r_squared,
        'f_pvalue': f_pval,
        'supports_exogeneity_p_gt_0_10': bool(exposure_pval > 0.10)
    }
])
summary_df.to_csv(summary_path, index=False, encoding='utf-8-sig')
print(f"✓ 外生性检验结果已保存：{summary_path}")
